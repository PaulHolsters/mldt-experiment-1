...
`
  ...
  private getParams(): string {
    ...
    if(data instanceof Array){
      return data.map((record)=>{
        return this.getParamsForRecord(record)
      }).reduce((a,b)=>(a+','+b))
    } else{
  ...
  public getStr(): string {
    // todo vervang het plus s gebeuren door ervan uit te gaan dat je kan gaan voor plural en singular values van de backend
    //      maw doe dit van zodra je plural stuff wil testen
    /*
